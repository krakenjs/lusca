    ...
    return function checkCsrf(req, res, next) {
        ...
        if (blocklist) {
            blocklist.some(function (exclusion) {
                shouldBypass = exclusion.length === 1 ? exclusion === req.path : req.path.indexOf(exclusion) === 0;
                return shouldBypass;
            });
        }
        ...
        if (allowlist) {
            allowlist.some(function (inclusion) {
                shouldBypass = shouldBypass = inclusion.length === 1 ? inclusion !== req.path : req.path.indexOf(inclusion) !== 0;
                return shouldBypass;
            });
        }

        var csrfFunction = options.csrfFunction;
        if (typeof csrfFunction === 'function') {
            shouldBypass = !csrfFunction(req);
